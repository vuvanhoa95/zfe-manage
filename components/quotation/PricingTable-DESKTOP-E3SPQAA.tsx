...
import { QuotationLineInput, OutsourceLineInput } from '@/types/quotation';
import { calculateQuotationTotals } from '@/lib/utils';
import { formatVND, numberToVietnameseWords } from '@/lib/number-to-words-vn';
import OutsourceCostDetail from './OutsourceCostDetail';
                            ...
                            return lines.map((line, index) => {
                                ...
                                return (
                                    <React.Fragment key={index}>
                                        ...
                                        {/* AI Pricing Suggestion Row */}
                                        {line.title && line.title.trim() && line.isChargeable && (
                                            <tr>
                                                <td colSpan={8} className="px-3 py-0 pb-2">
                                                    <AIPricingSuggestion
                                                        itemTitle={line.title}
                                                        customerId={undefined} // Will be passed from QuotationEditor context
                                                        onApply={(price) => updateLine(index, 'unitPrice', price)}
                                                    />
                                                </td>
                                            </tr>
                                        )}
                    ...
                    <div className="bg-yellow-50 p-4 rounded-xl border border-yellow-200 space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div className="col-span-full">
                                <OutsourceCostDetail
                                    value={outsourceLines}
                                    onChange={onOutsourceLinesChange || (() => {})}
                                    totalCost={outsourceCost || 0}
                                    onTotalCostChange={onOutsourceCostChange || (() => {})}
                                />
                            </div>
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="relative flex-1">
                                    <input
                                        type="number"
                                        value={taxRate ? Number((taxRate * 100).toFixed(2)) : ''}
                                        onChange={(e) => {
                                            const val = e.target.value;
                                            const rate = val ? parseFloat(val) / 100 : 0;
                                            onTaxRateChange?.(rate);
                                            onTaxCostChange?.(totalBeforeVat * rate);
                                        }}
                                ...
                                {commissionType === 'percentage' ? (
                                    <>
                                        <div className="relative flex-1">
                                            <input
                                                type="number"
                                                value={commissionRate ? Number((commissionRate * 100).toFixed(2)) : ''}
                                                onChange={(e) => {
                                                    const val = e.target.value;
                                                    const rate = val ? parseFloat(val) / 100 : 0;
                                                    onCommissionRateChange?.(rate);
                                                    onCommissionCostChange?.(totalBeforeVat * rate);
                                                }}
                                                className="w-full px-3 py-2 border border-yellow-300 rounded-lg bg-white text-zf-text-primary font-bold text-sm"
                                                placeholder="0"
                                                step="any"
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    <input
                                        type="number"
                                        value={profitRate ? Number((profitRate * 100).toFixed(2)) : ''}
                                        onChange={(e) => {
                                            const val = e.target.value;
                                            const rate = val ? parseFloat(val) / 100 : 0;
