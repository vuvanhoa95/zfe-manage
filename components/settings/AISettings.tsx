...
type AISettingsData = {
    ...
    // BIM ISO 19650
    companyCode: string;        // Mã công ty (VD: ZFE) dùng để đặt tên file BIM
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <p className="text-sm text-gray-500">Cấu hình các tính năng AI tích hợp trong hệ thống</p>
                </div>
                <button
                    onClick={handleSave}
            ...
            {/* ── BIM ISO 19650 Naming ──────────────────────────────────────── */}
            <div className="border border-gray-200 rounded-xl overflow-hidden">
                <div className="flex items-start justify-between p-4 bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 rounded-xl">
                    <div className="flex items-start gap-3">
                        <span className="text-2xl">🏷️</span>
                        <div>
                            <h3 className="font-bold text-gray-900 text-base">BIM ISO 19650 — Mã công ty</h3>
                            <p className="text-xs text-gray-600 mt-0.5">Mã định danh công ty dùng để tạo tên file BIM chuẩn ISO 19650</p>
                        </div>
                    </div>
                </div>
                <div className="p-5 space-y-3">
                    <div className="flex items-center gap-4">
                        <div className="flex-1">
                            <label className="block text-sm font-semibold text-gray-700 mb-1">Mã công ty (Company Code)</label>
                            <p className="text-xs text-gray-500 mb-2">2-5 ký tự, viết hoa. VD: ZFE, ARCH, BIM</p>
                            <input
                                type="text"
                                value={aiSettings.companyCode}
                                onChange={(e) => update('companyCode', e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5))}
                                placeholder="ZFE"
                                maxLength={5}
                                className="w-40 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm font-mono uppercase tracking-widest"
                            />
                        </div>
                        <div className="text-sm text-gray-600 bg-indigo-50 rounded-xl p-3 flex-1">
                            <p className="text-xs font-semibold text-indigo-700 mb-1">📋 Ví dụ tên file BIM:</p>
                            <p className="font-mono text-xs text-indigo-800">{aiSettings.companyCode || 'ZFE'}_PRJ2026001_STRU_01BS</p>
                            <p className="font-mono text-xs text-indigo-800">{aiSettings.companyCode || 'ZFE'}_PRJ2026001_ARCH_02-05FL</p>
                            <p className="font-mono text-xs text-indigo-800">{aiSettings.companyCode || 'ZFE'}_PRJ2026001_MEP_RF</p>
                        </div>
                    </div>
                </div>
            </div>
